...
class Report extends CI_Controller {
	...
	public function pendapatan(){
		...
		$this->load->view('kpa/report/pendapatan',$data);
    ...
    public function penggunaan_anggaran(){
		...
		$this->load->view('kpa/template/header',$data);
		$this->load->view('kpa/report/penggunaan_anggaran',$data);
		$this->load->view('kpa/template/footer');
    }
    public function json_penggunaan_anggaran(){
		...
		foreach ($get_data as $key => $value) {
			...
			$isi['action'] =	'
			<a href="'.site_url('kpa_side/detail_belanja/'.md5($value->id_belanja)).'" class="btn btn-outline btn-circle red btn-sm blue"><i class="fa fa-share"></i> Detail </a>
								';
			$data_tampil[] = $isi;
		}
		$results = array(
	...
	public function import_serapan(){
		...
		if($this->upload->do_upload('fmasuk')){
			...
			echo "<script>window.location='".base_url()."kpa_side/penggunaan_anggaran/'</script>";
		}else{
	...
	public function detail_belanja()
	{
		...
		$this->load->view('kpa/template/header',$data);
		$this->load->view('kpa/report/detail_belanja',$data);
		$this->load->view('kpa/template/footer');
	}
